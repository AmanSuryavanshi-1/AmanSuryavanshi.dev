...
const HeaderSocial: FC<HeaderSocialProps> = ({ className = '', isInverted = false }) => {
  const socialIcons = [
      ...
      {socialIcons.map(({ Icon, href, label }) => (
        <motion.a
          ...
          className={`relative text-2xl transition-colors ${
            isInverted 
              ? 'text-sage-300 hover:text-lime-400' 
              : 'text-forest-800 dark:text-sage-100 hover:text-lime-500 dark:hover:text-lime-400'
          }`}
        ...
        >
          <motion.div
            className={`absolute inset-0 rounded-full ${
              isInverted 
                ? 'bg-lime-400/10' 
                : 'bg-forest-700/20 dark:bg-lime-500/20'
            }`}
            initial={{ scale: 0 }}
