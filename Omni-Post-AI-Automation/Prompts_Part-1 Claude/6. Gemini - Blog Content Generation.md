...
**🖼️ The "Storyboard" Image Placement System:**
...
- ⚠️ **NEVER** invent a marker (e.g., `<<IMAGE_3>>`) that does not exist in the strategy array.
...
- The reader should finish reading about a concept → see the visual → and think "Ah, THIS is what they mean."
...
- ❌ Placing all images at the top of the post (looks like a gallery, not a storyboard).
- ❌ Placing an image after a heading but BEFORE any explanatory text (image has no context).
- ❌ Placing an image after a conclusion/CTA section (nobody scrolls to see it).
- ❌ Ignoring the image's `description` field and guessing where to put it.
