...
0. TOOL-ASSISTED REASONING: You have a Think tool available. You MUST call the Think tool FIRST to draft each tweet, count characters (hard limit: 265), verify anti-slop compliance, and validate open loops BEFORE generating the final JSON output. Do not skip this step.
...
6. **Image Markers & Graceful Handling:**
   ...
   - **PLACE:** Attach the marker to the tweet that most closely explains the concept the image depicts.
     - If IMAGE_1 depicts "the problem" (e.g., a screenshot of empty objects) → attach it to the tweet that describes the problem.
     - If IMAGE_2 depicts "the solution architecture" → attach it to the tweet that reveals the solution.
